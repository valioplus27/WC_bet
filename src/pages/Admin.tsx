...
function FeedbackLine({ feedback }: { feedback: Feedback | null }) {
  ...
  return (
    <p className={`text-sm ${feedback.kind === 'error' ? 'text-red-400' : 'text-pitch-400'}`} role="status">
      {feedback.message}
    </p>
  )
}
    ...
    <div className="mx-auto max-w-4xl space-y-8">
      <div>
        ...
        <p className="text-sm text-slate-400">
          Correct match results and configure the tournament bet. All data syncs run automatically.
        </p>
      </div>

      {/* Auto-sync status notice */}
      <div className="rounded-xl border border-pitch-600/30 bg-pitch-600/10 px-4 py-3">
        <p className="text-sm font-semibold text-pitch-400">All syncs are automated ✓</p>
        <ul className="mt-1 space-y-0.5 text-xs text-slate-400">
          <li>📅 Fixtures &amp; standings — every 1 minute via pg_cron</li>
          <li>🔴 Live events &amp; stats — every 2 minutes (skips when no match is live)</li>
          <li>📈 Predictions model — every hour</li>
          <li>👥 Squads &amp; coaches — daily at 03:00 UTC</li>
          <li>📊 Team analytics — daily at 04:00 UTC</li>
        </ul>
      </div>
  ...
  return (
    <section className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="text-base font-semibold text-slate-100">Match results</h2>
        <input
          type="search"
          value={query}
      ...
      {error && <p className="rounded-md bg-red-950/60 px-4 py-3 text-sm text-red-400">Couldn't load matches: {error}</p>}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="rounded-md bg-pitch-600 px-3 py-2 text-xs font-semibold text-white shadow-none transition hover:bg-pitch-700 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {saving ? 'Saving…' : 'Save'}
          </button>
        </div>
...
function TournamentConfigSection() {
  ...
  return (
    <section className="space-y-3">
      <h2 className="text-base font-semibold text-slate-100">Tournament bet settings &amp; results</h2>
      {loading ? (
        <Spinner label="Loading tournament settings…" />
      ) : !config ? (
        <p className="rounded-md bg-red-950/60 px-4 py-3 text-sm text-red-400">
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 rounded-xl border border-surface-4/70 bg-surface-2 p-5 shadow-none">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label htmlFor="podiumPoints" className="block text-sm font-semibold text-slate-100">
        ...
        <div>
          <label htmlFor="scorerPoints" className="block text-sm font-semibold text-slate-100">
      ...
      <fieldset className="space-y-3 border-t border-surface-4/40 pt-4">
        <legend className="text-sm font-semibold text-slate-100">Official tournament results</legend>
        <p className="text-xs text-slate-400">
      ...
      <button
        type="submit"
        disabled={saving}
        className="w-full rounded-md bg-pitch-600 px-4 py-2 text-sm font-semibold text-white shadow-none transition hover:bg-pitch-700 disabled:cursor-not-allowed disabled:opacity-50"
      >
