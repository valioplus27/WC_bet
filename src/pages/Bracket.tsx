...
function ProbBar({
  ...
  return (
    <div className="mt-3 space-y-1.5">
      {favouriteLabel ? (
        <p className="text-[11px] font-semibold text-slate-400">{favouriteLabel}</p>
      ) : (
      ...
      <div className="flex justify-between text-[10px] tabular-nums text-slate-400">
        <span className="font-semibold text-blue-400">{hw}%</span>
...
export default function Bracket() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-10">
      <div>
        <h1 className="text-xl font-bold text-slate-100">Knockout Bracket</h1>
      ...
      {predictions.length > 0 && (
        <section className="space-y-4 rounded-xl border border-surface-4 bg-surface-2 p-6 shadow-none">
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            <div className="rounded-lg border border-surface-4/40 bg-surface-1 px-3 py-3">
            ...
            <div className="rounded-lg border border-surface-4/40 bg-surface-1 px-3 py-3">
              <p className={`text-2xl font-bold ${avgBrier !== null && avgBrier < 0.2 ? 'text-pitch-400' : 'text-slate-100'}`}>
            ...
            <div className="rounded-lg border border-surface-4/40 bg-surface-1 px-3 py-3">
              <p className={`text-2xl font-bold ${avgLogLoss !== null && avgLogLoss < 0.6 ? 'text-pitch-400' : 'text-slate-100'}`}>
          ...
          {resolvedPreds.length > 0 && (
            <p className="text-xs text-slate-400">
              Based on <span className="font-semibold text-slate-300">{resolvedPreds.length}</span> resolved knockout match{resolvedPreds.length === 1 ? '' : 'es'}.
