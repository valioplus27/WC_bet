  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-xs text-slate-500">
        <span>{homeTeam}</span>
        <span className="font-semibold text-slate-900">xG</span>
        <span>{awayTeam}</span>
      </div>
      ...
      <div className="flex justify-between text-xs tabular-nums text-slate-400">
        <span className="text-blue-600 font-semibold">{homeXG.toFixed(2)}</span>
...
export default function MatchAnalysis() {
  ...
  if (!matchExtId) {
    return (
      <div className="mx-auto max-w-4xl space-y-6">
        <h1 className="text-xl font-bold text-slate-900">Match Analysis</h1>
        <p className="text-sm text-slate-500">
          ...
          <code className="rounded bg-slate-100 px-1 text-xs">?match=sb_43_&lt;id&gt;</code> query param.
        </p>
        <p className="text-sm">
          <Link to="/admin" className="font-medium text-pitch-700 hover:underline">
  ...
  if (error) {
    return <p className="rounded-md bg-red-50 px-4 py-3 text-sm text-red-700">Error: {error}</p>
  }
    ...
    return (
      <div className="mx-auto max-w-4xl space-y-4">
        ...
        <p className="rounded-lg border border-dashed border-slate-300 bg-white px-4 py-12 text-center text-sm text-slate-500">
          No data found for <code className="rounded bg-slate-100 px-1">{matchExtId}</code>.
          Run the StatsBomb ingestion from the{' '}
          <Link to="/admin" className="font-medium text-pitch-700 hover:underline">Admin page</Link> first.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-10">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <h2 className="mb-3 text-sm font-semibold text-slate-900">Expected goals (xG)</h2>
      ...
      <section className="space-y-6">
        <h2 className="text-sm font-semibold text-slate-900">Shot maps — attacking half</h2>
      ...
      {(homeNetwork || awayNetwork) && (
        <section className="space-y-6">
          <div>
            <h2 className="text-sm font-semibold text-slate-900">Passing networks</h2>
      ...
      {homeNetwork && awayNetwork && (
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="mb-3 text-sm font-semibold text-slate-900">Tactical fingerprint</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-left text-xs font-semibold uppercase tracking-wide text-slate-400">
                  <th className="py-2 pr-4">Metric</th>
                  <th className="py-2 pr-4 text-blue-600">{homeTeam}</th>
