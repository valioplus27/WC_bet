    ...
    <div className="flex min-h-screen items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {mode === 'magic-link' && <MagicLinkPanel onBack={() => setMode('password')} />}
...
// ----------------------------------------------------------------------------
// Magic link — the account-creation path. No password to invent up front;
// clicking the emailed link both proves the address is real and creates the
// account (via the on_auth_user_created trigger). A password can be added
// later through "Forgot password?", which Supabase treats the same whether
// you're setting one for the first time or replacing one you forgot.
// ----------------------------------------------------------------------------
function MagicLinkPanel({ onBack }: { onBack: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <p className="text-sm text-slate-600">
        New here? Enter your email and we'll send a link that signs you in and creates your account — no password to
        invent right now. You can add one any time from "Forgot password?" on the sign-in screen.
      </p>
      <EmailField value={email} onChange={setEmail} />
...
// ----------------------------------------------------------------------------
// Reset / set password — the bridge from "magic-link only" to "has a
// password". Supabase's resetPasswordForEmail handles both setting a password
// for the first time and replacing a forgotten one identically: either way it
// emails a recovery link that signs the user in and fires a PASSWORD_RECOVERY
// event, which useAuth/AuthProvider catches globally and routes to
// <SetNewPassword> regardless of which page the link redirects to.
// ----------------------------------------------------------------------------
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <p className="text-sm text-slate-600">
        ...
        adding one to an account you've only ever signed into with a magic link.
      </p>
      <EmailField value={email} onChange={setEmail} />
      <FieldError message={error} />
