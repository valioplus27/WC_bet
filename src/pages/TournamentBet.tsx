...
export default function TournamentBet() {
  ...
  if (!config) {
    return <p className="rounded-md bg-red-950/60 px-4 py-3 text-sm text-red-400">Tournament configuration hasn't been set up yet — ask an admin.</p>
  }
  ...
  return (
    <div className="mx-auto max-w-xl space-y-6">
      <div>
        <h1 className="text-lg font-bold text-slate-100">Tournament bet</h1>
        <p className="text-sm text-slate-500">
          Pick the podium and the Golden Boot winner — once, before the tournament starts. No edits after lock.
        </p>
      </div>

      <div className={`rounded-lg border px-4 py-2.5 text-sm ${locked ? 'border-surface-4 bg-surface-1 text-slate-400' : 'border-pitch-200 bg-pitch-600/10 text-pitch-800'}`}>
        {locked ? (
...
function TournamentBetForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-xl border border-surface-4/70 bg-surface-2 p-5 shadow-none">
      ...
      <div>
        <label htmlFor="topScorer" className="block text-sm font-semibold text-slate-100">
      ...
      {feedback && (
        <p className={`text-sm ${feedback.kind === 'error' ? 'text-red-600' : 'text-pitch-400'}`} role="status">
          {feedback.message}
        </p>
      )}
...
function LockedView({ bet, config }: { bet: TournamentBetRow | null; config: TournamentConfig }) {
  ...
  return (
    <div className="space-y-3 rounded-xl border border-surface-4/70 bg-surface-2 p-5 shadow-none">
      ...
      <div className="border-t border-surface-4/40 pt-3 text-sm">
        {bet.points_awarded !== null ? (
          <p className="font-semibold text-slate-100">
            Total: <span className="text-pitch-400">{bet.points_awarded} points</span>
          </p>
        ) : (
  ...
  return (
    <div className="flex items-center justify-between gap-3 text-sm">
      ...
      <span className="flex items-center gap-2">
        <span className="font-semibold text-slate-100">{pick}</span>
...
function Indicator({ correct }: { correct: boolean }): ReactNode {
  return correct ? (
    <span className="rounded-full bg-pitch-600/10 px-2 py-0.5 text-xs font-semibold text-pitch-400">✓ correct</span>
  ) : (
    <span className="rounded-full bg-surface-3 px-2 py-0.5 text-xs font-medium text-slate-400">✗ incorrect</span>
  )
}
